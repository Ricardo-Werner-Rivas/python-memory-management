...
# Import "currentframe" from inspect
...
class Pointer(Generic[object_type]):
    ...
    These pointers are completely safe and do not work internally as C's pointers.
    
    When a Pointer instance is created, though it stores a value, it "points" to an specific reference.
    ...
    Non-mutable objects still change their memory adresses when values change but the original object always share memory adress with the value of the pointer.
    ...
    ## Currently supported
    Both global and local variables con be pointed at.
    
    Literals are **not** supported because of it being useless.
    Only use for inserting a literal instead of a referenced value could be for the class program to display all the references that points to that literal
    and bind the instance to the desired option.
    """
